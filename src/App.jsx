...
function App() {
  ...
  const filterBayMainCategory = (category) => {
    const filterProducts = products.filter(
  ...
  return (
    <div className="text-white  container bg-[url('/bg-des.jpg')] bg-cover  bg-center   relative">
      ...
      {products.length > 0 ? (
        ...
        <div className="md:w-[450px]  ">
          <div className="bg-[#7b5d499e] border-[1px] text-[15px]  justify-between p-2  border-b-[1px]">
              ...
              {categories.map((category) => {
                return (
                  <li
                    ...
                    key={category.id}
                    className="text-[16px] hover:text-[#DCA57E] cursor-pointer border-[1px] rounded-[15px] py-2 px-10 "
                  >
                    {category.name}
                  </li>
                );
            ...
            <ul className="flex gap-2 mt-2">
              {categoryClick?.subCategories?.map((sub, index) => {
                return (
                  <li
                    onClick={() => filterProduct(sub.name)}
                    key={index}
                    className="text-[13px] hover:text-[#DCA57E] cursor-pointer border-[1px] rounded-[15px] py-2 px-2 "
                  >
                    {sub.name}
                  </li>
                );
       ...
       <svg
          aria-hidden="true"
          class="w-[20vh] h-[20vh] text-gray-200 animate-spin dark:text-gray-600 fill-[#DCA57E]"
          viewBox="0 0 100 101"
          fill="none"
